...
views_days = pd.pivot_table(df_time_diff_yr, index='days_published', values='Views',
                            aggfunc=[np.mean, np.median, lambda x: np.percentile(x, 80)]).reset_index()
views_days.columns = ['days_published', 'mean_views', 'median_views', '80pct_views']
views_days['20pct_views'] = pd.pivot_table(df_time_diff_yr, index='days_published', values='Views',
                                           aggfunc=[lambda x: np.percentile(x, 20)]).reset_index()['<lambda>']
...
if add_sidebar == 'Aggregate Metrics':
    ...
    for i in metric_median6mo.index:
        with columns[count]:
            delta = (metric_median6mo[i] - metric_median12mo[i]) / metric_median12mo[
                i]  # shows us increasing or decreasing value
    ...
    st.dataframe(df_agg_diff_final.style.applymap(style_negative, props='color:red;').applymap(style_positive,
